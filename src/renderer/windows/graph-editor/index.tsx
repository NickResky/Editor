...
import { Button, Divider, ButtonGroup, Switch, Popover, Position, Menu, MenuItem, MenuDivider, Toaster, Intent } from "@blueprintjs/core";
...
export interface IGraphEditorWindowState {
    ...
    playing: boolean;
    /**
     * Defines wether or not the generated code should be used.
     */
    useGeneratedCode: boolean;
}
...
export default class GraphEditorWindow extends React.Component<IGraphEditorWindowProps, IGraphEditorWindowState> {
    ...
    public render(): React.ReactNode {
        ...
        const graph =
            <Menu>
                <Switch label="Compile" style={{ paddingTop: "7px" }} checked={this.state.useGeneratedCode} onChange={(e) => this.setState({ useGeneratedCode: e.currentTarget.checked })} />
            </Menu>;
            ...
            <>
                <div className="bp3-dark" style={{ width: "100%", height: "80px", backgroundColor: "#444444" }}>
                    <ButtonGroup style={{ paddingTop: "4px" }}>
                        ...
                        <Popover content={graph} position={Position.BOTTOM_LEFT}>
                            <Button icon="graph" rightIcon="caret-down" text="Graph"/>
                        </Popover>
                    </ButtonGroup>
                    <Divider />
                    <ButtonGroup style={{ width: "100%", transform: "translate(50%)" }}>
    ...
    public start(): void {
        ...
        if (this.state.useGeneratedCode) {
            const mesh = this.preview.getMesh();
            const clone = mesh.clone(mesh.name);

            mesh.getScene().meshes.pop();
            this.graph.start(clone);
            this._generateCode();
        } else {
            this.graph.start(this.preview.getMesh());
        }
    ...
    private _generateCode(): void {
        const output = GraphCodeGenerator._GenerateCode(this.graph.graph!);
        if (output.error) {
            output.error.node.color = "#662222";
        } else {
            const str = GraphCodeGenerator.GenerateCode(this.graph.graph!);
            const mesh = this.preview.getMesh();
            const scene = mesh.getScene();

            const Module = require("module");
            const module = new Module();
            module._compile(str, Tools.RandomId());

            if (!module.exports || !module.exports.default) { return; }

            const prototype = module.exports.default.prototype;
            for (const key in prototype) {
                if (!prototype.hasOwnProperty(key) || key === "constructor") { continue; }
                mesh[key] = prototype[key].bind(mesh);
            }
